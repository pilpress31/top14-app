...
function PremiumDropdown({ label, value, onChange, options }) {
  ...
  return (
    <div className="relative w-full">
      ...
      {/* Menu */}
      {open && (
        <div className="absolute mt-2 w-full bg-white border rounded-lg shadow-lg z-50 animate-[fadeIn_0.15s_ease-out]">
          {options.map((opt) => (
            <div
              key={opt.value}
              onClick={() => {
...
export default function MaCagnotte() {
  ...
  // ------------------------------------------------------
  // 1️⃣ États principaux (inchangés, fidèles à ton PDF)
  // ------------------------------------------------------
  const [user, setUser] = useState(null);
  const [userCredits, setUserCredits] = useState(null);
  const [paris, setParis] = useState([]);              // Liste des paris enrichis
  const [transactions, setTransactions] = useState([]); // Transactions (v2)
  ...
  // ------------------------------------------------------
  // 2️⃣ Tri + filtre (fidèle à ton fichier)
  // ------------------------------------------------------
  ...
  const sortLabels = {
    ...
    ancien: "Ancien → Récent",
  };

  // ------------------------------------------------------
  // 3️⃣ Hook premium : récupération des données API v2
  //     → transactions + bets
  // ------------------------------------------------------
  const {
  ...
  // ------------------------------------------------------
  // 4️⃣ Charger l'utilisateur (inchangé)
  // ------------------------------------------------------
  useEffect(() => {
  ...
  // ------------------------------------------------------
  // 5️⃣ Synchronisation : quand le hook renvoie les données
  //     → on met à jour paris + transactions
  // ------------------------------------------------------
  useEffect(() => {
    if (!apiTransactions || !apiBets) return;
    ...
    // On arrête le loading global
    setLoading(false);
  }, [apiTransactions, apiBets]);

  // ------------------------------------------------------
  // 6️⃣ Gestion des erreurs du hook
  // ------------------------------------------------------
  if (apiError) {
    return (
      <div className="p-6 text-center text-red-600">
        Erreur lors du chargement des données : {apiError}
      </div>
    );
  }
...
// Cette partie s’exécute après que le hook a chargé les données
useEffect(() => {
  if (!apiBets || apiBets.length === 0) {
    // Si aucun pari, on remet tout à zéro
    setStats({
      totalBets: 0,
      pendingBets: 0,
      wonBets: 0,
  ...
  // ------------------------------------------------------
  // 2️⃣ Extraction des bonus & distributions (transactions)
  // ------------------------------------------------------
  const totalBonus = apiTransactions.filter((t) => t.type === "bonus" || t.type === "bonus_exact_score").length;

  const nbDistributions = apiTransactions.filter((t) => t.type === "monthly_distribution").length;

  // ------------------------------------------------------
  // 3️⃣ Mise à jour des stats globales
  // ------------------------------------------------------
  setStats({
...
}, [apiBets, apiTransactions]);

// ======================================================
// ===============  FIN PARTIE 3 / 6  ===================
// ======================================================
// ======================================================
// ======  PARTIE 4 — NAVIGATION & UTILITAIRES  =========
// ======================================================
...
// 📊 Calculs statistiques dérivés (winRate, ROI)
const winRate =
  stats.totalBets > 0
...
const teams = Array.from(
  new Set(
    paris
      .filter((t) => t.matches || t.bets?.matches)
      .flatMap((t) => {
        const match = t.matches || t.bets?.matches;
        return [
...
return (
  <div className="min-h-screen bg-rugby-white pb-24">
    ...
    {activeTab === "overview" ? (
      <div className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="group relative">
                  ...
                  <div className="invisible group-hover:visible absolute left-0 top-6 w-64 bg-gray-900 text-white text-xs rounded-lg p-3 shadow-lg z-50">
                    ...
                    <br />
                    ((Total gagné - Total misé) / Total misé) x 100
                    <br />
                    <br />
                    <strong>Positif :</strong> Vous gagnez plus que vous misez
                    <br />
                    <strong>Négatif :</strong> Vous perdez de l&apos;argent
                    <br />
                    <strong>Exemple :</strong> ROI +20% = 20% de profit sur vos
                    mises
                  </div>
                </div>
              </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <p className="text-xs text-blue-700">
            <strong>Astuce :</strong> Diversifiez vos paris et ne misez jamais
            plus de 10% de votre cagnotte sur un seul match !
          </p>
        </div>
        ...
        <button
          onClick={() =>
            navigate("/pronos", { state: { activeTab: "mes-paris" } })
          }
      ...
      // La vue "transactions" arrive en PARTIE 6
      null
    )}
  </div>
);
