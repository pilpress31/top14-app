...
// Groupé par date. Indice favori + « Pourquoi ce pronostic ? ».
// Pas de Compo/blessés (aucune source de compos internationales).
// Couleurs : charte MONDE (vert émeraude / émeraude)
// ============================================

import { useState, useEffect, useRef } from 'react';
import { Calendar, ChevronDown, ChevronUp, Brain } from 'lucide-react';
...
// ── Carte d'un match (composant top-level → état conservé, pas de remount) ──
function CarteMonde({ m, conseilOpen, onToggleConseil }) {
        ...
        <div className="flex flex-col items-center">
          {hasPred ? (
            <>
              ...
              {indice != null && (
                <span className="text-[9px] text-gray-500 mt-0.5">Confiance {indice}%</span>
              )}
            </>
          ) : (
...
export default function AlgoPronosMondeTab() {
  ...
  const [openConseilId, setOpenConseilId] = useState(null);
  ...
  const toggleConseil = (id) => setOpenConseilId(prev => (prev === id ? null : id));
              ...
              <div className="divide-y divide-gray-200">
                {matchsDate.map(m => (
                  <CarteMonde
                    key={m.match_id}
                    m={m}
                    conseilOpen={openConseilId === m.match_id}
                    onToggleConseil={() => toggleConseil(m.match_id)}
                  />
                ))}
              </div>
            )}
