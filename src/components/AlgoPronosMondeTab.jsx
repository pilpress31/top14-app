...
//          GET /api/monde/insights?equipe_dom=&equipe_ext=
//          GET /api/monde/historique?equipe=&adversaire=
// Vitrine des prédictions (pas de betting → page Paris). Groupé par date.
// Panneaux : Indice favori, « Pourquoi ce pronostic ? », Duel & Forme,
//            Historique des confrontations. (Pas de Compo/blessés : aucune
//            source de compositions internationales.)
// ============================================
...
function SerieBlocs({ forme }) {
  if (!Array.isArray(forme) || forme.length === 0) {
    return <span className="text-[11px] italic text-gray-400">Pas assez de matchs internationaux récents</span>;
  }
...
// ── Panneau Duel & Forme (GET /api/monde/insights) ──
function InsightsMonde({ match, isOpen, onToggle }) {
  ...
  const handleToggle = async () => {
    ...
    if (!isOpen && !data && !loading) {
      ...
      try {
        const url = `${API_BASE}/api/monde/insights?equipe_dom=${encodeURIComponent(match.equipe_domicile)}&equipe_ext=${encodeURIComponent(match.equipe_exterieure)}`;
  ...
  return (
    <div className="mt-3 border-t pt-3" style={{ borderColor: MONDE_BORDER }}>
      <RubriqueHeader
        theme={RUBRIQUE_THEMES.monde}
        icon={BarChart2}
        label="Duel & Forme"
        isOpen={isOpen}
        loading={loading}
      ...
      {isOpen && (
        <div className="mt-3 space-y-3">
          {error && <p className="text-xs text-center py-2 italic" style={{ color: MONDE_GREEN }}>{error}</p>}
          {loading && <div className="flex justify-center py-4"><Loader2 className="w-6 h-6 animate-spin" style={{ color: MONDE_EMERAUDE }} /></div>}
          {data && !loading && (
            <>
              {data.duel_texte && (
                <div className="rounded-lg p-3" style={{ backgroundColor: MONDE_SOFT, border: `1px solid ${MONDE_BORDER}` }}>
                  <p className="text-[11px] leading-relaxed" style={{ color: MONDE_GREEN }}>{data.duel_texte}</p>
                </div>
              )}

              {nb >= 3 && (
                <div className="rounded-lg p-3" style={{ backgroundColor: MONDE_SOFT, border: `1px solid ${MONDE_BORDER}` }}>
                  <p className="text-[10px] font-bold uppercase tracking-wide mb-2" style={{ color: MONDE_GREEN }}>
                    ⚔️ Face-à-face — {nb} matchs
                  </p>
                  <div className="mb-3">
                    <div className="flex justify-between text-[10px] mb-1" style={{ color: MONDE_GREEN }}>
                    ...
                    <div className="flex h-2 rounded-full overflow-hidden">
                      <div style={{ width: `${Math.round((data.h2h.victoires_dom / nb) * 100)}%`, backgroundColor: MONDE_GREEN }} />
                      <div style={{ width: `${Math.round((data.h2h.nuls / nb) * 100)}%`, backgroundColor: '#9CA3AF' }} />
                      <div style={{ width: `${Math.round((data.h2h.victoires_ext / nb) * 100)}%`, backgroundColor: MONDE_EMERAUDE }} />
                    </div>
                    <div className="flex justify-between text-[10px] mt-0.5 font-bold">
                      <span style={{ color: MONDE_GREEN }}>{Math.round((data.h2h.victoires_dom / nb) * 100)}%</span>
                      <span style={{ color: MONDE_EMERAUDE }}>{Math.round((data.h2h.victoires_ext / nb) * 100)}%</span>
                    </div>
                  </div>
                  {data.h2h.moyenne_pts_dom != null && (
                    <div className="rounded p-2 text-center" style={{ backgroundColor: '#FFFFFF', border: `1px solid ${MONDE_BORDER}` }}>
                      <p className="text-[11px] font-bold" style={{ color: MONDE_GREEN }}>
              ...
              {[
                ...
                <div key={idx} className="rounded-lg p-3" style={{ backgroundColor: MONDE_SOFT, border: `1px solid ${MONDE_BORDER}` }}>
                  <p className="text-[10px] font-bold uppercase tracking-wide mb-2" style={{ color: MONDE_GREEN }}>
                    🏉 {eq.label} — 5 derniers matchs internationaux
                  </p>
...
// ── Panneau Historique des confrontations (GET /api/monde/historique) ──
function HistoriqueConfrontationsMonde({ match, isOpen, onToggle }) {
  ...
  const handleToggle = async () => {
    ...
    if (!isOpen && !confrontations && !loading) {
      setLoading(true); setError(null);
      try {
        const res = await axios.get(`${API_BASE}/api/monde/historique`, {
        ...
        console.error('Erreur historique confrontations MONDE:', e);
  ...
  return (
    <div className="px-3 py-3">
      ...
      {hasPred && (
        <>
          {winnerName && (
            <div className="mt-1 flex items-center justify-center gap-1.5 text-[11px] font-semibold" style={{ color: MONDE_GREEN }}>
              <Brain className="w-3.5 h-3.5" /> Favori : {winnerName}
            </div>
          )}
          {indice != null && <BarreIndiceFavori pct={indice} variant="monde" />}
          {PHASE_FINALE(m.phase) && (
            <p className="text-[9px] text-gray-400 italic text-center -mt-2 mb-1">Prédiction sur le temps réglementaire (80 min)</p>
          )}
