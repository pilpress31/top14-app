// ============================================
// MES PARIS - EUROPEAN CHALLENGE CUP
// Source : GET /api/ecc/user/bets/detailed
// Vue Supabase : v_user_bets_ecc_detailed
// Couleurs : vert #2E7D32 + bronze #CD7F32
// ============================================

import { useState, useEffect, useRef } from 'react';
import { Coins, TrendingUp, TrendingDown, Trophy, Clock, FileText, Target, Shield } from 'lucide-react';
...
const PHASE_FINALE_ROUNDS = ['16e de finale', '8e de finale', 'Quart de finale', 'Demi-finale', 'Finale'];

export default function MesParisEccTab() {
    ...
    if (targetMatchId && paris.length > 0) {
      setTimeout(() => {
        ...
        if (element) {
          ...
          element.style.setProperty('--tw-ring-color', ECC_BRONZE);
          setTimeout(() => {
  ...
  const loadData = async () => {
    try {
      ...
      // Crédits + total gagné GLOBAL (tous championnats)
      try {
      ...
      // Paris ECC (source : v_user_bets_ecc_detailed)
      try {
        const parisResponse = await axios.get(`${API_BASE}/api/ecc/user/bets/detailed`, {
          headers: { 'x-user-id': user.id }
        });
        setParis(parisResponse.data.bets || []);
      } catch (error) {
        console.error('Erreur chargement paris ECC:', error);
        setParis([]);
      }
    } catch (error) {
      console.error('❌ Erreur globale chargement données ECC:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="animate-spin rounded-full h-12 w-12 border-b-4" style={{ borderColor: ECC_VERT }}></div>
      </div>
    );
  }
  ...
  const bandeauStyle = {
    background: `linear-gradient(to right, ${ECC_VERT}, ${ECC_BRONZE})`
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Badge championnat ECC */}
      <div
        className="flex items-center justify-center gap-2 px-4 py-2 rounded-lg w-fit mx-auto"
        style={{ backgroundColor: ECC_VERT }}
      >
        <Shield className="w-4 h-4" style={{ color: ECC_BRONZE }} />
        <span className="text-xs font-bold uppercase tracking-wide" style={{ color: ECC_BRONZE }}>
          Paris Challenge Cup
        </span>
      </div>
      ...
      {parisFiltered.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-6 text-center border border-rugby-gray">
          <p className="text-gray-500">
            {filter === 'pending' && 'Aucun pari Challenge Cup en cours'}
        ...
        <div className="space-y-3">
          {parisFiltered.map(bet => {
            ...
            const isPhaseFinale = PHASE_FINALE_ROUNDS.includes(bet.phase);
                ...
                <div className={`px-4 py-2 flex items-center justify-between ${
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      ...
                      style={{ backgroundColor: ECC_VERT, color: ECC_BRONZE }}
                    >
                      🛡️ CHALL.
                    </span>
                    {isPhaseFinale && (
                    ...
                    <div className="mb-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div
                          ...
                          style={{ backgroundColor: '#EEF7EE', borderColor: ECC_VERT }}
                        >
                          <p className="text-[10px] font-semibold mb-1 flex items-center gap-1" style={{ color: ECC_VERT }}>
                            <Target className="w-3 h-3" /> Ton pronostic
                          </p>
                          {bet.bet_type === 'WINNER_FT' ? (
                            <p className="text-base font-bold text-center" style={{ color: ECC_VERT }}>
                              🎯 {bet.winner_predit === 'DOM' ? (teamDom?.name || 'Domicile')
                                : bet.winner_predit === 'EXT' ? (teamExt?.name || 'Extérieur')
                                : 'Match nul'}
                            </p>
                          ) : (
                            <p className="text-xl font-bold text-center" style={{ color: ECC_VERT }}>
