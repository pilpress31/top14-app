// ============================================
// HISTORIQUE - CHALLENGE CUP (ECC)
// Source : GET /api/ecc/historique  (mode tous-matchs)
// Filtres : année / équipe + recherche ; pagination.
// Prono correct = vainqueur prédit == vainqueur réel.
// (Scores ECC déjà finaux → pas de score final a.p.)
// Couleurs : charte ECC (vert challenge / bronze)
// ============================================
...
export default function HistoriqueEccTab() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`${API_BASE}/api/ecc/historique`, { params: { limit: 6000 } });
        const data = res.data.matchs || [];
        data.sort((a, b) => new Date(b.date_match) - new Date(a.date_match));
        setMatchs(data);
      } catch (e) {
        console.error('Erreur chargement historique ECC:', e);
        setMatchs([]);
      } finally {
        setLoading(false);
      }
  ...
  const filtered = useMemo(() => {
    ...
    return r;
  }, [matchs, annee, equipe, search]);

  // reset page si les filtres réduisent la liste
  useEffect(() => { setPage(1); }, [annee, equipe, search]);
  ...
  return (
    <div className="space-y-3 pb-32">
      ...
      <button
        ...
        style={{ backgroundColor: 'rgba(46,125,50,0.08)', color: ECC_GREEN }}
      >
      ...
      {showFilters && (
        <div className="flex flex-col gap-2 bg-white rounded-lg border border-gray-200 p-3">
          ...
          <input
            type="text" placeholder="Rechercher (équipe, année)…"
              ...
              <div key={m.match_id} className="w-full bg-white rounded-lg shadow-sm border border-gray-200 py-3 px-3">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {PHASE_FINALE(m.phase) && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                        style={{ backgroundColor: ECC_ACCENT, color: '#FFFFFF' }}>🏆 {m.phase}</span>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 items-start">
                  ...
                  <div className="flex flex-col items-center justify-center gap-1">
                    ...
                    <div className="text-2xl font-bold" style={{ color: ECC_GREEN }}>
      ...
      {/* Pagination — pill flottant FIXE au-dessus de la barre de nav (teinte ECC) */}
      {totalPages > 1 && (
        <div
        ...
        >
          <div
            ...
            style={{ pointerEvents: 'auto', backgroundColor: '#0c1322', border: '1px solid rgba(205,127,50,0.45)' }}
          >
            <button
              ...
              style={{ backgroundColor: ECC_GREEN, color: '#FFFFFF' }}
            >◀ Précédent</button>

            <span className="px-1 text-xs font-semibold whitespace-nowrap" style={{ color: ECC_ACCENT }}>
