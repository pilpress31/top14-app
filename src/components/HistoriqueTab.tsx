...
export default function HistoriqueTab({ headerVisible = true, isD2 = false }: HistoriqueTabProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Pagination */}
      <div className="fixed left-0 w-full flex justify-center items-center gap-4 py-2 shadow-md z-50" style={isD2 ? { backgroundColor: '#00174D', borderTop: '1px solid rgba(192,192,192,0.3)', bottom: 'calc(var(--bottom-nav-total, 90px) + 8px)' } : { backgroundColor: '#FFFFFF', bottom: 'calc(var(--bottom-nav-total, 90px) + 1px)' }}>
        <button
          onClick={() => {
            if (isD2) {
